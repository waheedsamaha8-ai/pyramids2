...
export default function App() {
  ...
  const refreshAllData = async () => {
    ...
    try {
      ...
      if (loadedComplaints) setComplaints(loadedComplaints);
      ...
      if (loadedMessages) offlineSync.saveCachedData('chat_messages', loadedMessages);
      ...
      if (loadedComplaints) offlineSync.saveCachedData('public_complaints', loadedComplaints);
  ...
  const addResident = (resident: Resident) => {
    ...
    if (isOnline) {
      googleApi.addResidentSheet(resident)
        .then(() => {
          googleApi.saveAppConfig(updatedConfig).catch(() => {});
          refreshAllData();
        })
        .catch(err => {
  ...
  const setAllResidents = (newResidents: Resident[]) => {
    ...
    // For batch operations, we'll replace the local state and sync.
    ...
    // since we are replacing the entire list now.
    offlineSync.clearResidentActionsFromQueue();
    
    if (isOnline) {
      googleApi.setAllResidentsSheet(newResidents)
        .then(() => refreshAllData())
        .catch(err => {
          logError(err, 'setAllResidents');
          // For batch, we don't have a specific queue type yet, but saving cache is done.
        });
    }
  };
  ...
  const updateBuildingLayout = (layout: FloorConfig[]) => {
    ...
    if (isOnline) {
      googleApi.saveAppConfig(updatedConfig).catch(err => logError(err, 'saveBuildingLayout'));
    }
  };
  ...
  const editResident = (resident: Resident) => {
    ...
    if (isOnline) {
      googleApi.editResidentSheet(resident)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const deleteResident = (id: string) => {
    ...
    if (isOnline) {
      googleApi.deleteResidentSheet(id)
        .then(() => {
          googleApi.saveAppConfig(updatedConfig).catch(() => {});
          refreshAllData();
        })
  ...
  const deletePayment = (id: string) => {
    ...
    if (isOnline) {
      googleApi.deletePaymentSheet(id)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const deleteExpense = (id: string) => {
    ...
    if (isOnline) {
      googleApi.deleteExpenseSheet(id)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const handleAddRule = (e: React.FormEvent) => {
    ...
    if (isOnline) {
      googleApi.saveBuildingRulesSheet(updatedRules)
        .then(() => refreshAllData())
        .catch(() => offlineSync.enqueueAction('UPDATE_RULES', updatedRules));
    } else {
      offlineSync.enqueueAction('UPDATE_RULES', updatedRules);
    }
  };
  ...
  const addMaintenanceRequest = (req: MaintenanceRequest) => {
    ...
    if (isOnline) {
      googleApi.addMaintenanceRequestSheet(req)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const updateMaintenanceRequest = (id: string, updates: Partial<MaintenanceRequest>) => {
    ...
    if (target) {
      if (isOnline) {
        googleApi.editMaintenanceRequestSheet(target)
          .then(() => refreshAllData())
          .catch(err => {
  ...
  const deleteMaintenanceRequest = (id: string) => {
    ...
    if (role === 'RESIDENT') {
      const isMyReq = (flatNumber !== undefined && target.flatNumber === flatNumber) ||
        (Boolean(user?.displayName) && Boolean(target.residentName) && target.residentName.trim().toLowerCase() === user.displayName.trim().toLowerCase());
      if (!isMyReq) {
        return;
      }
    }
    ...
    offlineSync.saveCachedData('maintenance', updated);
    if (isOnline) {
      googleApi.deleteMaintenanceRequestSheet(id)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const addCraftsman = (craftsman: Craftsman) => {
    ...
    if (isOnline) {
      googleApi.addCraftsmanSheet(craftsman)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const deleteCraftsman = (id: string) => {
    ...
    offlineSync.saveCachedData('craftsmen', updated);
    if (isOnline) {
      googleApi.deleteCraftsmanSheet(id)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const editCraftsman = (updatedCraftsman: Craftsman) => {
    ...
    offlineSync.saveCachedData('craftsmen', updated);
    if (isOnline) {
      googleApi.editCraftsmanSheet(updatedCraftsman)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const votePoll = (pollId: string, optionId: string, email: string) => {
    ...
    if (updatedPoll) {
      if (isOnline) {
        googleApi.editPollSheet(updatedPoll)
          .then(() => refreshAllData())
          .catch(err => {
  ...
  const deletePoll = (pollId: string) => {
    ...
    offlineSync.saveCachedData('polls', updated);
    if (isOnline) {
      googleApi.deletePollSheet(pollId)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const addDecision = (decision: AdminDecision) => {
    ...
    if (isOnline) {
      googleApi.addAdminDecisionSheet(decision)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const editDecision = (decision: AdminDecision) => {
    ...
    offlineSync.saveCachedData('admin_decisions', updated);
    if (isOnline) {
      googleApi.editAdminDecisionSheet(decision)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const deleteDecision = (id: string) => {
    ...
    offlineSync.saveCachedData('admin_decisions', updated);
    if (isOnline) {
      googleApi.deleteAdminDecisionSheet(id)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const addBuildingEvent = (newEvent: BuildingEvent) => {
    ...
    if (isOnline) {
      googleApi.addEventSheet(newEvent)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const updateBuildingEvent = (id: string, updates: Partial<BuildingEvent>) => {
    ...
    if (target) {
      if (isOnline) {
        googleApi.editEventSheet(target)
          .then(() => refreshAllData())
          .catch(err => {
  ...
  const handleSaveAppConfig = (updatedConfig: AppConfig) => {
    ...
    if (residentsChanged) {
      ...
      offlineSync.saveCachedData('residents', updatedResidents);
      if (isOnline) {
        googleApi.setAllResidentsSheet(updatedResidents).catch(() => {});
      }
    }
  ...
  const handleAddCommentToComplaint = (complaintId: string, text: string) => {
    ...
    if (targetComplaint) {
      ...
      if (isOnline) {
        googleApi.editComplaintSheet(targetComplaint)
          .then(() => refreshAllData())
          .catch(err => {
  ...
  const handleDeleteComplaint = (complaintId: string) => {
    ...
    if (role === 'RESIDENT') {
      const isMyComp = (Boolean(user?.displayName) && Boolean(target.residentName) && target.residentName.trim().toLowerCase() === user.displayName.trim().toLowerCase()) ||
        (flatNumber !== undefined && target.flatNumber !== undefined && target.flatNumber === flatNumber);
      if (!isMyComp) {
        return;
      }
    }
    ...
    offlineSync.saveCachedData('public_complaints', updated);
    if (isOnline) {
      googleApi.deleteComplaintSheet(complaintId)
        .then(() => refreshAllData())
        .catch(err => {
  ...
  const handleDeleteChatMessage = (messageId: string) => {
    ...
    offlineSync.saveCachedData('chat_messages', updated);
    if (isOnline) {
      googleApi.setAllChatMessagesSheet(updated)
        .catch(err => {
          logError(err, 'handleDeleteChatMessage');
          offlineSync.enqueueAction('SET_ALL_CHAT', updated);
        });
    } else {
      offlineSync.enqueueAction('SET_ALL_CHAT', updated);
    }
      ...
      <main className="flex-1 max-w-full mx-auto w-full px-1 sm:px-1.5 py-3 space-y-3.5" dir="rtl">
        ...
        {activeTab === 'maintenance' && (
          <MaintenanceRequests 
            ...
            role={role}
            flatNumber={flatNumber || config.adminResidentProfile?.flatNumber}
            userName={
              (role === 'RESIDENT' && config.adminResidentProfile?.name)
                ? config.adminResidentProfile.name
                : (currentResidentObj?.name || user?.displayName || 'ساكن')
