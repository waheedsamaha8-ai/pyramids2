...
export const VotingPolls: React.FC<VotingPollsProps> = ({
  ...
  return (
    <div className="w-full space-y-4 text-right animate-fade-in" id="polls-panel" dir="rtl">
      ...
      <CommunityHeader
        ...
        badge={activeTab === 'polls' ? `${polls.length} استبيان` : `${decisions.length} قرار معتمد`}
        counts={communityCounts || {
          polls: polls.length,
          decisions: decisions.length,
        }}
        actionButton={
          activeTab === 'polls' && role !== 'RESIDENT' && role !== 'ASSISTANT' ? (
            <button
              type="button"
          ...
          ) : activeTab === 'decisions' && role !== 'RESIDENT' && role !== 'ASSISTANT' && onAddDecision ? (
            <button
              type="button"
              onClick={() => {
            ...
            >
              <Plus className="w-4 h-4" />
              <span>{showAddDecisionForm ? 'إلغاء النموذج' : 'إصدار قرار إداري جديد'}</span>
            </button>
          ) : undefined
        }
      />
      ...
      <div className="flex items-center gap-1.5 p-1 bg-slate-100 dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700/80 rounded-xl w-fit">
        ...
        >
          ...
          <span className={`text-[10px] font-extrabold px-1.5 py-0.2 rounded-md ${activeTab === 'polls' ? 'bg-blue-800 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
            {polls.length}
          </span>
        </button>
        ...
        >
          ...
          <span className={`text-[10px] font-extrabold px-1.5 py-0.2 rounded-md ${activeTab === 'decisions' ? 'bg-blue-800 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300'}`}>
            {decisions.length}
          </span>
        </button>
      </div>
        ...
        <div className="space-y-3">
          ...
          {polls.length === 0 ? (
            <div className="py-12 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-[#111a2e]">
              ...
              <h4 className="text-xs sm:text-sm font-black text-slate-800 dark:text-slate-200">لا توجد استبيانات أو مواضيع تصويت حالياً</h4>
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 font-bold">يمكن لرئيس الاتحاد أو المدير إضافة تصويت جديد في أي وقت.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {polls.map((poll) => {
                ...
                const hasVoted = !!(poll.userVotes && poll.userVotes[userEmail]);
                const votedOptionId = poll.userVotes ? poll.userVotes[userEmail] : undefined;
                          ...
                          return (
                            <div key={opt.id} className="relative">
                              ...
                              {hasVoted || !isActive ? (
                                ...
                                <button
                                  type="button"
                                  onClick={() => onVote(poll.id, opt.id, userEmail)}
                      ...
                      <div className="space-y-2 w-full">
                        <div className="bg-slate-50 dark:bg-slate-800/60 p-2.5 sm:p-3 rounded-xl space-y-1 border border-slate-100 dark:border-slate-700/80">
                          ...
                          <div className="text-xl font-black text-slate-900 dark:text-slate-100">
                            {totalVotesCount} <span className="text-xs font-bold text-slate-500">وحدة</span>
                          </div>
                        </div>
