  ...
  const handleSendReminder = (item: typeof residentsWithDebt[0], target: 'owner' | 'tenant' | 'both' = 'owner') => {
    ...
    if (target === 'both') {
      ...
      sendSingleWhatsApp(
        ...
        financials.expectedDues,
        financials.totalPaid
      );

      // Send to tenant after a short delay if tenant exists
      if (hasTenant) {
        setTimeout(() => {
          sendSingleWhatsApp(
            resident.tenantName || 'المستأجر',
      ...
      markReminded(`${resident.id}_owner`);
      markReminded(`${resident.id}_tenant`);
      markReminded(`${resident.id}_both`);
