...
export const Chat: React.FC<ChatProps> = ({
  ...
  // Handle Image Selection for Complaints
  ...
  const handleSendChat = (e: React.FormEvent) => {
    ...
    onSendMessage(messageText.trim(), chatImageFile || undefined);
    setMessageText('');
          ...
          <div className="flex-1 overflow-y-auto p-3.5 sm:p-5 space-y-3 bg-slate-50/50 dark:bg-slate-900/30">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-3 py-12">
                ...
                <p className="text-sm font-black text-slate-600 dark:text-slate-300">لا توجد رسائل سابقة في الدردشة.</p>
                <p className="text-xs text-slate-400 dark:text-slate-500 font-bold">كن أول من يرحب بالجيران أو يرسل استفساراً أو صورة!</p>
              </div>
            ) : (
              messages.map((msg) => {
                  ...
                  >
                    ...
                    <div>
                      ...
                      {(() => {
                        ...
                        const canDeleteComplaint = role !== 'RESIDENT' || isMyComplaint;
